...
plotOverlapTable <- function(randomized.overlaps, id.metadata=NULL, id.groups=NULL,
  ...
  if(!is.null(id.groups)){
    ...
    ids <- strsplit(ids, split="-", fixed=T)
    ids <- unique(unlist(ids))
  }

  nids <- length(ids)
  pairwise_results$id1 <- unlist(lapply(strsplit(pairwise_results$ids, split="-", fixed=T), function(x) x[[1]]))
  pairwise_results$id2 <- unlist(lapply(strsplit(pairwise_results$ids, split="-", fixed=T), function(x) x[[2]]))
  ...
  if(type=="mean overlap"){
    ...
    overlap_range <- range(pairwise_results, na.rm=T)
  ...
  if(!is.null(sort.by) & !is.null(id.metadata)){
    vars1 <- unlist(sapply(ids1, function(x) id.metadata[,sort.by][id.metadata[,id.col]==x], simplify=F))
    vars2 <- unlist(sapply(ids2, function(x) id.metadata[,sort.by][id.metadata[,id.col]==x], simplify=F))
    ...
    rownames(pairwise_results) <- NULL
    colnames(pairwise_results) <- NULL
    digits <- max(.decimalPlaces(c(vars1, vars2)), na.rm=T)
  ...
  if(type=="significance"){
    ...
    if(full.scale==T){
      ...
      overlaps <- round(.rescale(overlaps, from=range(overlaps, na.rm=T), to=c(1, length(color.pal)-1)))
    }
  ...
  text(x=values$col, y=values$row, labels=values$value, font=values$font, cex=0.7, xpd=T, adj=0.5)
  if(!is.null(sort.by)){
  ...
  # add bounding box
  segments(x0=0.5, x1=n_cols+0.5, y0=0.5, xpd=T)
  segments(x0=0.5, x1=n_cols+0.5, y0=n_rows+0.5, xpd=T)
  segments(y0=0.5, y1=n_rows+0.5, x0=0.5, xpd=T)
  segments(y0=0.5, y1=n_rows+0.5, x0=n_cols+0.5, xpd=T)
  ...
  if(type=="significance"){
    ...
    overlap_digits <- max(.decimalPlaces(overlap_labs), na.rm=T)

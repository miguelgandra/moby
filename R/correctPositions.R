...
#' - A `SpatRaster` (terra; a `RasterLayer` is also accepted) with binary values or bathymetric data.
...
#' This parameter can be either 'land', 'water', or 'bathy'.
#' \itemize{
...
#' @param verbose Logical; print progress and a relocation summary to the console. Defaults to
...
correctPositions <- function(data,
  ...
  # accept a raster::RasterLayer for convenience, but process it with terra internally
  }else if(inherits(spatial.layer, "RasterLayer")){
  ...
  if(inherits(spatial.layer, "SpatRaster")){
    spatial.layer[is.na(spatial.layer)] <- 999999
  }
  ...
  if(inherits(spatial.layer, "SpatRaster")){
     ...
     spatial.layer <- terra::trim(spatial.layer)
     # check if there are any remaining NA values in the trimmed raster
     if(any(is.na(terra::values(spatial.layer)))){
       # initialize a variable to track whether NA values remain
       na_remaining <- TRUE
       # iterate until there are no NA values left in the raster
       while(na_remaining) {
         # fill the NA cells based on nearest neighbors (3x3 window); na.policy="only" fills only NA
         # cells. fillvalue=NA pads the edges with NA (dropped by na.rm), so border cells fill from
         # real neighbours only - matching raster::focal(pad=TRUE, padValue=NA); a numeric pad would
         # leak into the result and corrupt the sentinel restore below.
         spatial.layer <- terra::focal(spatial.layer, w=matrix(1,3,3), fun="modal", na.rm=TRUE, na.policy="only", fillvalue=NA)
         # check if there are still NA values remaining
         na_remaining <- any(is.na(terra::values(spatial.layer)))
       }
     }
     # convert all temporary dummy values back to NA
     spatial.layer[spatial.layer==999999] <- NA
   }
  ...
  if(inherits(spatial.layer, "SpatRaster")){
    ...
    # water cells (value 0) as points -> candidate relocation targets
    land_mask <- terra::as.points(terra::ifel(raster_layer==0, 1, NA))
  ...
  if(plot){
    ...
    if(inherits(spatial.layer, "SpatRaster")){
      ...
      spatial.layer <- terra::crop(spatial.layer, terra::ext(plot_bbox[c("xmin","xmax","ymin","ymax")]))
      ...
      spatial.layer[spatial.layer<0] <- 0
      if(raster.type=="land") spatial.layer <- terra::ifel(is.na(spatial.layer), 1, NA)
      # create a mask for NA values in the raster
      na_mask <- is.na(spatial.layer)
      # convert the NA mask into polygons for contouring
      na_polygons <- terra::as.polygons(terra::ifel(na_mask==1, 1, NA), dissolve=TRUE)
      ...
      terra::plot(spatial.layer, col=color_pal, main="Results", axes=FALSE, legend=FALSE)
      # overlay the NA areas with a grey color for better visual distinction
      terra::plot(na_mask, col=c(NA, "grey"), add=TRUE, legend=FALSE)
      # add a contour around the NA areas
      terra::plot(na_polygons, border="black", lwd=0.2, add=TRUE)
      # add a contour at the depth threshold, if specified,
      if(depth.threshold>0) terra::contour(spatial.layer, levels=depth.threshold, col="black", lwd=0.6, add=TRUE)

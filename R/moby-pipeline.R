...
#'   \item{\strong{3. Enrich} - \code{\link{matchTags}}, \code{\link{matchDeployments}}}{Fold a side
#'     table into the detections. `matchTags()` resolves transmitter codes to animals;
#'     `matchDeployments()` matches detections to deployment windows and back-fills station names and
#'     coordinates. They key on different columns (transmitter vs receiver) and are
#'     \strong{order-independent}. Both change rows and columns only: neither attaches study
#'     metadata, and both return the same class they were given.}
...
#'   \item{\strong{5. Declare} - \code{\link{as_moby}}}{The package's only constructor, and the one
#'     place study metadata enters a dataset: which column plays which role, plus `id.groups`,
#'     `epsg.code`, `land.shape`, and - via `tags` - each animal's tagging date and nominal delay.
#'     Idempotent, so call it whenever you want to add metadata without losing what is already
#'     attached.}
#' }
#'
...
#'   `matchTags()`         \tab recommended \tab IDs stay as exported transmitter codes \cr
...
#'   `as_moby()`           \tab \strong{essential in practice} \tab `filterDetections()` \strong{errors} without tagging dates; no `id.groups`, projected CRS or land-aware distances \cr
#' }
#'
...
#'   \item \strong{Declare before you filter.} \code{\link{filterDetections}} needs tagging dates,
#'     which reach the dataset through `as_moby(det, tags = tags)` (or its own `tagging.dates`
#'     argument). The two enrich steps themselves may run in either order.
#' }
#'
...
#'   \item \code{\link{matchTags}} leaves `NA` IDs (with a warning) for transmitters that are not in

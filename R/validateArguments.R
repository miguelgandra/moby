...
#'   \item{\code{ncol}}{Checks that the \code{ncol} parameter is correctly formatted as a single positive integer.}
#' }
#'
#' @note
...
.validateArguments <- function() {
  ...
  parent_frame <- sys.frame(sys.nframe()-1L)
  args <- mget(names(formals(sys.function(sys.parent()))), parent_frame)
  data <- args[["data"]]

  # resolve NULL column/metadata arguments from the mobyData object (if supplied) or from
  # the canonical defaults, and push the resolved values back into the calling function's
  # frame so that its body can continue to use id.col, datetime.col, etc. unchanged
  if (inherits(data, "data.frame")) {
    resolved <- .resolveArgs(data, args)
    for (nm in names(resolved)) {
      if (!identical(resolved[[nm]], args[[nm]])) {
        args[[nm]] <- resolved[[nm]]
        assign(nm, resolved[[nm]], envir = parent_frame)
      }
    }
  }
  ...
  if ("variables" %in% names(args)) {
    ...
    if(!is.null(all_variables) && calling_fun!="plotChronogram"){
      for(v in seq_along(all_variables)){
  ...
  if ("diel.lines" %in% names(args)) {
    ...
    if(length(diel.lines)!=1 || !diel.lines %in% c(0, 2, 4)) errors <- c(errors, "Invalid 'diel.lines' specified. Accepted values are '0' (no lines), '2' (sunrise and sunset) or '4' (dawn, sunrise, sunset, dusk).")
  }
  ...
  if ("polygons" %in% names(args)) {
    ...
    if(length(polygons)!=1 || !polygons %in% c(FALSE, "diel","season")) errors <- c(errors,"Invalid 'polygons' specified. Accepted values are: 'diel', 'season' or 'FALSE' (disabled).")
  }
  ...
  if ("cores" %in% names(args)) {
    cores <- args$cores
    if (!is.numeric(cores) || length(cores)!=1 || cores < 1 || cores %% 1 != 0) {
      errors <- c(errors, "The 'cores' parameter must be a single positive integer.")
    } else if (cores>1){
  ...
  ##############################################################################
  # Validate 'ncol' argument (number of panel columns) #########################
  if ("ncol" %in% names(args)) {
    ncol <- args$ncol
    if (!is.null(ncol)){
      if (!is.numeric(ncol) || length(ncol) > 1 || ncol %% 1 != 0) errors <- c(errors, "The 'ncol' parameter (number of panel columns) must be a single positive integer.")
    }
  }
...
.checkAnimalParams <- function(argument, arg_label, expected_class=c("POSIXct", "numeric"),
                               data, id_col, allow.missing=FALSE) {
  ...
  # case 1: a single UNNAMED value -> replicate for all IDs. A single NAMED value is treated as a
  # (partial) named vector below, so e.g. max.speed = c(A = 5) is matched to ID "A" only, not applied
  # to every individual.
  if (length(argument) == 1 && is.null(names(argument))) {
    processed_argument <- rep(argument, nlevels(data[[id_col]]))

    # case 2: a named value, or multiple values
  } else if (length(argument) >= 1) {
    ...
    if (is.null(names(argument))) {
      ...
      if (length(missing_ids) > 0 && !allow.missing) {
        errors <- c(errors, paste0("The following IDs are missing in ", arg_name, ": ",
                                   paste(missing_ids, collapse = ", ")))
      } else {
        # reorder the argument to match the ID levels; when allow.missing is TRUE, IDs absent
        # from a named vector become NA (the caller treats NA as "skip this individual")
        processed_argument <- unname(argument[levels(data[[id_col]])])
      }
    }
  }

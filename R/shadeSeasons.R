...
shadeSeasons <- function(start.time, end.time, interval, color.pal=c("white", "grey96", "grey83", "grey90"),
  ...
  seasons_start <- stats::aggregate(complete_seqs$timebin, by=list(complete_seqs$season), min, simplify=F)
  seasons_end <- stats::aggregate(complete_seqs$timebin, by=list(complete_seqs$season), max, simplify=F)

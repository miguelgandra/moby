...
#' }
#' When the input is a \code{\link{mobyData}}, the result is also a `mobyData`: the input's metadata
#' (CRS/`epsg.code`, tagging dates, `id.groups`, land layer) is carried forward - so downstream
#' spatial analyses such as \code{\link{calculateStepDistances}} and \code{\link{calculateUDs}}
#' inherit it automatically. (The raw datetime column does not survive aggregation, so `datetime.col`
#' is not retained.)
#'
#' @examples
#' data(rays)
...
calculateCOAs <- function(data,
  ...
  # (na.action=na.pass so that time bins are not silently dropped when some
  # coordinates are NA; the per-group mean still ignores NAs via na.rm=TRUE)
  ...
  # merge all data frames using Reduce and .joinKeep
  result <- Reduce(function(x, y) .joinKeep(x, y, by=c(id.col, timebin.col), type="left"), data_list)
  ...
  # removes the raw datetime column, so 'datetime.col' is dropped.

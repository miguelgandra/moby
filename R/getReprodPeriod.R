...
getReprodPeriod <- function(datetimes, spawning.start, spawning.end, format="%m", tz="UTC") {
  ...
  spawning.start <- as.POSIXct(spawning.start, format=format, tz=tz)
  spawning.end <- as.POSIXct(spawning.end, format=format, tz=tz)
  # handle spawning periods that wrap around the end of the year (start > end,
  # e.g. November to February): in that case a date is within the period if it is
  # after the start OR before the end
  wrap <- spawning.start > spawning.end
  in_period <- ifelse(wrap,
                      datetimes >= spawning.start | datetimes <= spawning.end,
                      datetimes >= spawning.start & datetimes <= spawning.end)
  reprod_period <- ifelse(in_period, "spawning", "resting")
